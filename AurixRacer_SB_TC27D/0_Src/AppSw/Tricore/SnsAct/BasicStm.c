...
void BasicStm_init(void)
{
    ...
    //ÇÉ ¸ðµå ¾Æ¿ôÇ² ¼³Á¤
    IfxPort_setPinMode(PORT00_0.port, PORT00_0.pinIndex, IfxPort_Mode_outputPushPullGeneral);
...
//    printf("BasicStm_run() called\n");
	...
	if(task_flag_100m == TRUE){
		...
		if((int)userStatus % 2 == 1) {
			//ledÀü¿ø on
			IfxPort_setPinState(PORT00_0.port, PORT00_0.pinIndex, IfxPort_State_high);
		}
		else {
			//Àü¿ø off
			IfxPort_setPinState(PORT00_0.port, PORT00_0.pinIndex, IfxPort_State_low);
		}
	}
